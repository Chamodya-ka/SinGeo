...
class SupervisedInfoNCE(nn.Module):
    def __init__(self, device, eps: float = 1e-8, singeo_replicate=False):
        super().__init__()
        self.eps = eps
        self.device=device
        self.singeo_replicate = singeo_replicate
    ...
    def forward(
        ...
        labels: torch.Tensor = None, bidirectional = True, same_domain = False                 
    ) -> torch.Tensor:
        ...
        if self.singeo_replicate and labels is None:
            logits_a2g = logits_g2a.T
            labels = torch.eye(len(logits_g2a), dtype=torch.long, device=self.device)
            loss1 = self._multi_positive_ce(logits_g2a, labels, self.eps, False)
            loss2 = self._multi_positive_ce(logits_a2g, labels, self.eps, False)
