...
def composite_contrast_loss(
    ...
    if isinstance(loss_function, SupervisedInfoNCE):
        loss_q2r = loss_function(features_q1, features_r1, logit_scale, target_matrix)
        
        # contrast query to reference features
        loss_q2q = loss_function(features_q1, features_q1, logit_scale, q2q_target, bidirectional=False, same_domain=True)
        loss_r2r = loss_function(features_r1, features_r1, logit_scale, r2r_target, bidirectional=False, same_domain=True)


        return loss_q2r + 0.5 * loss_q2q + 0.5 * loss_r2r
...
def train_contrast_singeo(train_config, model, dataloader, loss_function, optimizer, scheduler=None, scaler=None):
    ...
    if train_config.verbose:
        ...
        bar = dataloader
    # for loop over one epoch
    for query_images, reference_images, target_matrix, q2q_target, r2r_target in bar:
        ...
        if scaler:
            with autocast():
                ...
                target_matrix = target_matrix.to(train_config.device) #[A*B, A*B]
                ...
                q2q_target = q2q_target.to(train_config.device) # [B*A, B*A]
                r2r_target = r2r_target.to(train_config.device)
                # ids [B*A,]
                # debug - save the first 2 images of the first batch to check if they are loaded correctly
                # if step == 1:
                #     for x in range(len(query_images)):
                #         torchvision.utils.save_image(query_images[x], f"debug/query_image_{x}.png")
                #         torchvision.utils.save_image(reference_images[x], f"debug/reference_image_{x}.png")
